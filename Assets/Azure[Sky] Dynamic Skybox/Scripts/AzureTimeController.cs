...
namespace UnityEngine.AzureSky
{
    ...
    public class AzureTimeController : MonoBehaviour
    {
        ...
        private Transform m_moonTransform = null;

        [Tooltip("The directional light used to apply the sun and moon lighting to the scene.")] [SerializeField]
        private Transform m_directionalLight = null;

        [Tooltip(
            "If selected, the prefab will follow this transform in the scene, usually the main camera is used here.")]
        [SerializeField]
        ...
        [SerializeField]
        private AzureTimeSystem m_timeSystem = AzureTimeSystem.Simple;

        [Tooltip("The direction in which the time of day will flow.")] [SerializeField]
        private AzureTimeDirection m_timeDirection = AzureTimeDirection.Forward;

        [Tooltip("The loop in which the calendar will perform the day changes.")] [SerializeField]
        private AzureDateLoop m_dateLoop = AzureDateLoop.Off;

        [Tooltip(
            "The current 'time position' in the day-night cycle. Note that this may not represent the correct time of day.")]
        [SerializeField]
        private float m_timeline = 6.0f;

        [Tooltip("The hour converted from the time of day.")] [SerializeField]
        private int m_hour = 6;

        [Tooltip("The minute converted from the time of day.")] [SerializeField]
        private int m_minute = 0;

        [Tooltip("The day used by the calendar and date system.")] [SerializeField]
        private int m_day = 1;

        [Tooltip("The month used by the calendar and date system.")] [SerializeField]
        private int m_month = 1;

        [Tooltip("The year used by the calendar and date system.")] [SerializeField]
        private int m_year = 2021;

        [Tooltip("The current selected calendar day.")] [SerializeField]
        private int m_selectedCalendarDay = 1;

        [Tooltip("The north-south angle of a position on the Earth's surface.")] [SerializeField]
        private float m_latitude = 0;

        [Tooltip("The east-west angle of a position on the Earth's surface.")] [SerializeField]
        private float m_longitude = 0;

        [Tooltip("Universal Time Coordinated.")] [SerializeField] private float m_utc = 0;

        [Tooltip("Duration of the day-night cycle in minutes.")] [SerializeField]
        private float m_dayLength = 24.0f;

        [Tooltip(
            "The minimum directional light altitude (0° - 90°). You can use this to avoid the shadows to get stretched when the sun is close to the horizon at sunset.")]
        [SerializeField]
        private float m_minLightAltitude = 0.0f;

        [Tooltip("Will the 'time of day' be evaluated based on the timeline or based on the day-night length curve?")]
        [SerializeField]
        private bool m_isTimeEvaluatedByCurve = false;

        [Tooltip("Duration of the day-night cycle in minutes.")] [SerializeField]
        private AnimationCurve m_dayLengthCurve = AnimationCurve.Linear(0, 0, 24, 24);

        [Tooltip("List storing all celestial bodies currently in use by the system.")] [SerializeField]
        private List<AzureCelestialBody> m_celestialBodiesList = new List<AzureCelestialBody>();

        [Tooltip("Event triggered when the minute changes.")] [SerializeField]
        private UnityEvent m_onMinuteChange = new UnityEvent();

        [Tooltip("Event triggered when the hour changes.")] [SerializeField]
        private UnityEvent m_onHourChange = new UnityEvent();

        [Tooltip("Event triggered when the day changes to the next day at midnight.")] [SerializeField]
        ...
        [Tooltip("The sun elevation in the sky.")] private float m_sunElevation = 0f;

        [Tooltip("The moon elevation in the sky.")] private float m_moonElevation = 0f;

        [Tooltip("The distance between Earth and the Sun.")] private float m_sunDistance = 0f;

        [Tooltip("The local direction of the Sun.")] private Vector3 m_sunLocalDirection;

        [Tooltip("The local direction of the Moon.")] private Vector3 m_moonLocalDirection;

        [Tooltip("The local direction of the Directional Light.")] private Vector3 m_lightLocalDirection;

        [Tooltip("The distance between Earth and the Moon.")] private float m_moonDistance = 0f;

        [Tooltip("The time interval the system should use for checking the custom event list.")] [SerializeField]
        private AzureEventScanMode m_customEventScanMode = AzureEventScanMode.ByMinute;

        [Tooltip("The list of custom events.")] [SerializeField]
        ...
        [Tooltip("Array with 42 numeric strings used to fill a calendar.")] [SerializeField]
        ...
        /// </summary>
        public void StartTimelineTransition(int hour, int minute, float speedMultiplier,
            AzureTimeDirection timeDir = AzureTimeDirection.Forward, int day = int.MaxValue, int month = int.MaxValue,
        ...
        private void ApplyTimelineTransitionForward()
        {
            ...
            if (m_hour >= m_timelineTransitionTargetHour)
            {
                ...
                {
                    if (m_timelineTransitionTargetDay == int.MaxValue &&
                        m_timelineTransitionTargetMonth == int.MaxValue &&
                        m_timelineTransitionTargetYear == int.MaxValue)
                    {
                    ...
                    {
                        if (m_timelineTransitionTargetMonth == int.MaxValue &&
                            m_timelineTransitionTargetYear == int.MaxValue)
                        {
                            CancelTimelineTransition();
                            return;
        ...
        private void ApplyTimelineTransitionBackward()
        {
            ...
            if (m_hour <= m_timelineTransitionTargetHour)
            {
                if (m_minute <= m_timelineTransitionTargetMinute)
                {
                    if (m_timelineTransitionTargetDay == int.MinValue &&
                        m_timelineTransitionTargetMonth == int.MinValue &&
                        m_timelineTransitionTargetYear == int.MinValue)
                    {
                        CancelTimelineTransition();
                        return;
                    }
                    ...
                    if (m_day <= m_timelineTransitionTargetDay)
                    {
                        if (m_timelineTransitionTargetMonth == int.MinValue &&
                            m_timelineTransitionTargetYear == int.MinValue)
                        {
                            CancelTimelineTransition();
                            return;
        ...
        private void EvaluateTimeOfDay()
        {
            ...
            string formattedTime = string.Format("{0:00}:{1:00}", m_hour, m_minute);

            OnTimeChanged?.Invoke(m_day, m_hour, m_minute);
        }
        ...
        {
            m_directionalLight.localRotation =
                Quaternion.LookRotation(m_sunElevation >= 0.0f ? m_sunLocalDirection : m_moonLocalDirection);
                ...
                case AzureTimeSystem.Realistic:
                    ...
                    Shader.SetGlobalMatrix(AzureShaderUniforms.StarfieldMatrix,
                        Matrix4x4.TRS(Vector3.zero,
                            Quaternion.Euler(90.0f - m_latitude, 0.0f, 0.0f) *
                            Quaternion.Euler(0.0f, m_longitude, 0.0f) * Quaternion.Euler(0.0f, LST * deg, 0.0f),
                            Vector3.one).inverse);
                                ...
                                case AzureCelestialBody.Type.Pluto:
                                    ...
                                    float pluto_lonecl = 238.9508f + 0.00400703f * d
                                        ...
                                        + 0.128f * Mathf.Sin(5 * P) - 0.034f * Mathf.Cos(5 * P)
                                                                    - 0.038f * Mathf.Sin(6 * P) +
                                        0.031f * Mathf.Cos(6 * P)
        ...
        {
            return Quaternion.Euler(0.0f, m_longitude, -m_latitude) *
                   Quaternion.Euler(((m_timeOfDay + m_utc) * 360.0f / 24.0f) - 90.0f, 180.0f, 0.0f);
        }
